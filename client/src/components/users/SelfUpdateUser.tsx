...
import { SelfUpdateUserViewModel } from "../../viewmodels/authenticated/states/users/self-update";

@observer
export class SelfUpdateUser extends React.Component<{viewModel: SelfUpdateUserViewModel}> {
  public render() {
    return (
      <Card
      ...
      >
        <Form model={this.props.viewModel} {...{onSubmit: this.onSubmit} as any}>
          ...
          <Form.Item label="Current password" required={true}>
            <Input
              type="password"
              value={this.props.viewModel.currentPassword}
              onChange={(value: any) => this.props.viewModel.currentPassword = value}
            />
          </Form.Item>
